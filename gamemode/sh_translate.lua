...
local debug_translation = CreateConVar("dd_debug_translation", 0, FCVAR_ARCHIVE + FCVAR_REPLICATED, "Shows unlocalized strings in current language"):GetBool()
cvars.AddChangeCallback("dd_debug_translation", function(cvar, oldvalue, newvalue)
	debug_translation = tobool( newvalue )
end)

if CLIENT then
	-- Need to make a new convar since gmod_language isn't sent to server.
	CreateClientConVar("gmod_language_rep", "en", false, true)
	CreateClientConVar("dd_language", "auto", true, false)
	...
	timer.Create("checklanguagechange", 1, 0, function()
		CurrentLanguage = GetConVar("dd_language"):GetString() == "auto" and GetConVar("gmod_language"):GetString() or GetConVar("dd_language"):GetString()
		if CurrentLanguage ~= GetConVar("gmod_language"):GetString() then
...
local function translateGet(id)
	if debug_translation then
		return translate.GetTranslations(CurrentLanguage)[id] or ("@"..id.."@")
	end
