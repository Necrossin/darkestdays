...
Perks = {
	...
	["martialarts"] = {Name = "Martial Arts", Special = "Weapon mod: Augments your fists and\nbreaks your fingers", Pr = "Cut people with bare hands\nDropkicks to the face", Co = "No primary weapon\nVulnerability after dropkick", Description = "Attack when sprint-jumping to perform\na drop kick.", OnSet = function(pl) if SERVER then for k,v in ipairs(pl:GetWeapons()) do if not v.IsMelee then v:Remove() break end end; pl:SelectWeapon( "dd_fists" ); pl:SendLua("LocalPlayer()._MartialArts = true") end end, OnReset = function(pl) if SERVER then pl:SendLua("LocalPlayer()._MartialArts = nil") end end},
	["dash"] = {Name = "Dash", Special = "Press sprint to dash in any direction", Pr = "Two-Handed weapons: press attack\nwhile dashing forward, to perform\na dash attack", Co = "Costs 25% of your mana per dash\nCosts even more mana per dash attack\n-15 max health",OnSet = function(pl) pl._DefaultHealth = pl._DefaultHealth - 15; if SERVER then pl:SendLua("LocalPlayer()._ShiftUses = 4") end end, OnReset = function(pl) if SERVER then pl:SendLua("LocalPlayer()._ShiftUses = nil") end end},
	["crow"] = {Name = "Crow Master",Mat = Material( "darkestdays/hud/cotn.png" ), Special = "Press sprint to become a crow\nfor a few seconds", Pr = "Doubles damage of Murder of Crows", Co = "Costs 50% of your mana\nYou can die in cramped areas",OnSet = function(pl) if SERVER then pl:SendLua("LocalPlayer()._ShiftUses = 2") end end, OnReset = function(pl) if SERVER then pl:SendLua("LocalPlayer()._ShiftUses = nil") end end},
}
