...
function SWEP:ViewModelDrawn()
	...
	for k, name in ipairs( self.vRenderOrder ) do
		...
		if (v.type == "Model" and ValidEntity(model)) then
			...
			/*if model:GetBoneName(0) == "static_prop" then
				matrix:Scale(Vector(v.size.x, v.size.y, v.size.z))
			else
				matrix:Scale(Vector(v.size.y, v.size.x, v.size.z))
			end
			model:EnableMatrix( "RenderMultiply", matrix )*/
			
			//if model:GetManipulateBoneScale(0) ~= v.size then
...
function SWEP:OpenBrowser( current, browse_type, callback )
	...
	if (browse_type == "model") then
		...
		zoomslider.ConVarChanged = function( panel, value )
